...
- [修改] HOME 欢迎页 image2 分元素重制 + 全层动效（worldcup/onboarding.jsx · worldcup/assets/welcome/）
  - 用 image2 生成 `image2-asset-sheet.png`，重新生成头像、足球、球场、语言气泡、彩带、白布幅、底部图标等元素；不使用用户原图整图贴底，也不做伪透明底。
  - 裁切并接入 7 个头像、足球、球场正式资源：`fan-br/jp/fr/es/mx/kr/de.png`、`ball.png`、`pitch-generated.png`。
  - E1 Splash 继续保持 HTML/CSS 分层：logo、跳过按钮、语言气泡、主标题布幅、倒计时、球场连线、聊天节点、CTA、底部卖点栏全部是独立 DOM。
  - 新增全层动画：天空呼吸、太阳光扫动、球场轻微漂移、灯光闪烁、彩带漂浮、纸屑循环下落、气泡浮动、白布幅摆动、标题呼吸、足球旋转、头像漂浮、聊天气泡浮动、虚线脉冲、CTA 呼吸、底栏入场。
  - 修复首帧风险：关键元素不再依赖 `opacity:0` 入场，避免低性能设备或 headless 截图时主标题/倒计时/CTA 短暂不可见。
  - 修复层级：球场层保持在底部，倒计时、头像、聊天气泡、CTA、底部卖点栏使用明确 `z-index`，避免遮挡。
  - 二次修正背景和彩带：新增 `wc-bg-photo` 真实球场光感层，顶部背景从扁平渐变改为蓝天 + 灯光 + 看台环；彩带从 8 条直线改为 12 条动态弧形 ribbon，按原图重排顶部左红、顶部蓝黄、右上红、右侧黄、底部绿蓝金粉红。
  - 三次修正背景：新增 image2 生成的 `background-plate.png` 纯背景底板（864×1821），直接承载蓝天、太阳光、看台、绿茵场透视和世界地图草纹；关闭旧 CSS 拼接天空/看台/蓝色场地区域，避免和原图不一致。
  - 四次修正标题：新增 image2 生成的 `title-banner.png` 独立标题布幅，替换原 HTML 拼接的白布幅、`HelloTalk`、`世界杯`、足球和副标题，保持整体漂浮动效。
  - 五次扩展后续页：新增 image2 生成的 `step-country.png`、`step-guide.png`、`step-highlights.png`、`step-activate-v2.png` 四张 864×1821 版面底图；E2-E5 统一改成第一张图同款竖版画布，底图负责版面氛围，HTML 继续负责准确文字和可点击控件。
  - 已部署到生产：`https://worldcup-jekae-bpehs0rdr-moons-projects-cc633ac1.vercel.app`，并 alias 到 `https://worldcup.jekae.com`。

## [2026-04-24]

- [新增] 项目根 backup/worldcup_母版_2026-04-24/ 母版备份（2.9 MB）
  - 冻结当前满意状态：入场漏斗 5 屏奶白糖果色系 + E1 彩虹硬阴影 START + 淡黄胶囊 + 静态 splash.png 底图（无动画层）
  - 内容：完整 worldcup/ 目录（含 assets/onb/splash.png 920×1400 + assets/onb/pitch.png + 所有 jsx 源文件）+ CHANGELOG_snapshot + README_snapshot
  - onboarding.jsx 验证：`rainbow` prop 保留（彩虹阴影）· `SplashAnimLayer` 清除（动画层已回滚）
  - 与 4.23 母版并列：backup/worldcup_母版_2026-04-23/（19 项 NavPanel 色条版）· backup/worldcup_母版_2026-04-24/（入场漏斗 5 屏版）
